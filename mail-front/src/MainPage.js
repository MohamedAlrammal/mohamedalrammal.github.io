...
function MainPage() {
  ...
  */
  ...
  const [sorting, setSorting] = useState(localStorage.getItem("sortingBy"));
  ...
  const increment = () => {
    setCount((count) => (count < 4 ? count + 1 : 0));
  };
  //sending data
  ...
  const fetchContacts = async () => {
    try {
      const response = await axios.post("http://localhost:8080/api/Mail/sortCo", `{"guest":"${user}"}`, {
        headers: {
          'Content-Type': 'application/json',
        },
        withCredentials: false
  ...
  useEffect(() => {
    setSorting(localStorage.getItem('sortingBy'));
    if (currentPage === 'inbox') {
      ...
      if (isSearch === false) {
        if(sorting === "date"){
          ...
          postData("sortDr")
          console.log("hello from sorted  draft")
        }
        }
  ...
  }, [contentArr, currentPage, sorting]);
  return (
