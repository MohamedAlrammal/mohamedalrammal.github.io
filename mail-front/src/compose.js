...
export default function Compose({jsonData, setCurrentPage, onClose, user, setJsonData}){
  ...
  async function postData(newJsonData)
  ...
  async function handleBack(e){
    ...
    if(form.files !== undefined){const files = Array.from(form.files.files);
     attachments = await Promise.all(
        files.map(async (file) => ({
            ...
            fileType: file.type,
            fileSize: file.size,
            fileContent: await encodeToBase64(file),
        }))
    );}
    newJsonData.attachment = attachments;
    console.log("HERE IS THE ATTACHMENTS:"+ newJsonData.attachments)
    newJsonData.sender = user;
    newJsonData.priority = jsonData.priority;
   ...
   newJsonData.attachment = attachments;
   if (newJsonData.attachment && newJsonData.attachment.length === 0) {
    newJsonData.attachment = null;
}
   newJsonData.priority = jsonData.priority;
    delete newJsonData.files
    newJsonData.sender = user
    newJsonData.date = format(new Date(), 'yyyy-MM-dd');
    console.log(newJsonData)
    let receivers = newJsonData.receiver.split(",");
    for(let i = 0;i < receivers.length;i++){
        newJsonData.receiver = receivers[i];
        postData(newJsonData)
    }
     //newJsonData.attachments = ; //fill it with attachments
